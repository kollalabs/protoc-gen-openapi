...
* `public_docs`: a build run with this tag generates only the methods that carry
  it. Every other method is left out. Use it for a service whose public spec is
  an allow-list.
* `postman`: a service's `service_params` headers are only added to builds run
  with a tag the service lists, so `build_tags: ["postman"]` adds them to the
  Postman build alone.
* `internal_docs`: a method that carries it is generated **only** by a build
  run with `build_tag=internal_docs`. It is left out of every other build —
  including one run with no build tag at all, which is how a public spec is
  normally produced. Use it for an endpoint that exists in the API but must not
